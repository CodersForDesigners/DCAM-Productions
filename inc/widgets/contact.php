...
if ( ! class_exists( 'CI_Widget_Contact' ) ) :
	class CI_Widget_Contact extends WP_Widget {
		...
		function __construct() {
			$widget_ops  = array( 'description' => esc_html__( 'Display a map and contact information.', 'vidiho-pro' ) );
			$control_ops = array();
			parent::__construct( 'ci-contact', esc_html__( 'Theme - Contact', 'vidiho-pro' ), $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			if ( $fields ) {
				...
				foreach ( $fields as $field ) {
					echo sprintf( '<li class="ci-contact-widget-item"><i class="%1$s"></i> %2$s</li>', $field['icon'], $field['title'] );
				}
				echo '</ul>';
			}
		...
		function form( $instance ) {
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'map_code' ) ); ?>"><?php esc_html_e( 'Map code (accepts HTML):', 'vidiho-pro' ); ?></label><textarea id="<?php echo esc_attr( $this->get_field_id( 'map_code' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'map_code' ) ); ?>" class="widefat"><?php echo esc_textarea( $map_code ); ?></textarea></p>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'contact_title' ) ); ?>"><?php esc_html_e( 'Contact title:', 'vidiho-pro' ); ?></label><input id="<?php echo esc_attr( $this->get_field_id( 'contact_title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'contact_title' ) ); ?>" type="text" value="<?php echo esc_attr( $contact_title ); ?>" class="widefat" /></p>
						...
						if ( ! empty( $fields ) ) {
							...
							for ( $i = 0; $i < $count; $i++ ) {
								...
								<div class="post-field">
									<label class="post-field-item"><?php echo wp_kses( sprintf( __( 'Icon (e.g. <code>fas fa-check</code> <a href="%s" target="_blank">Reference</a>):', 'vidiho-pro' ), 'https://fontawesome.com/cheatsheet' ), array( 'code' => array(), 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?>
										<input type="text" name="<?php echo esc_attr( $field_icon_name ); ?>" value="<?php echo esc_attr( $fields[ $i ]['icon'] ); ?>" class="widefat" />
									</label>

									<label class="post-field-item"><?php esc_html_e( 'Title:', 'vidiho-pro' ); ?>
		...
		protected function sanitize_contact_fields( $instance ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$new_fields[ $records_count ]['icon']  = sanitize_text_field( $icons[ $i ] );
