...
if ( ! class_exists( 'CI_Widget_Schedule' ) ) :
	class CI_Widget_Schedule extends WP_Widget {
		...
		function __construct() {
			$widget_ops  = array( 'description' => esc_html__( 'Display a schedule.', 'vidiho-pro' ) );
			$control_ops = array();
			parent::__construct( 'ci-schedule', esc_html__( 'Theme - Schedule', 'vidiho-pro' ), $widget_ops, $control_ops );
		}
		...
		function form( $instance ) {
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Text (accepts HTML):', 'vidiho-pro' ); ?></label><textarea id="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'text' ) ); ?>" class="widefat"><?php echo esc_textarea( $text ); ?></textarea></p>
						...
						if ( ! empty( $fields ) ) {
							...
							for ( $i = 0; $i < $count; $i++ ) {
								...
								<div class="post-field">
									<label class="post-field-item"><?php esc_html_e( 'Day:', 'vidiho-pro' ); ?>
										<input type="text" name="<?php echo esc_attr( $field_day_name ); ?>" value="<?php echo esc_attr( $fields[ $i ]['day'] ); ?>" class="widefat" />
									</label>

									<label class="post-field-item"><?php esc_html_e( 'Time:', 'vidiho-pro' ); ?>
