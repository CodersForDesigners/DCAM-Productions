...
if ( ! class_exists( 'CI_Widget_Latest_Post_Type' ) ) :
	class CI_Widget_Latest_Post_Type extends WP_Widget {
		...
		function __construct() {
			$widget_ops  = array( 'description' => esc_html__( 'Displays a number of the latest (or random) posts from a specific post type.', 'vidiho-pro' ) );
			$control_ops = array();
			parent::__construct( 'ci-latest-post-type', __( 'Theme - Latest Post Type', 'vidiho-pro' ), $widget_ops, $control_ops );
		}
		...
		function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'post_type' ) ); ?>"><?php esc_html_e( 'Select a post type to display the latest post from:', 'vidiho-pro' ); ?></label>
