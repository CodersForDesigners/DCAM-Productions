...
class Widget_Latest_Videos extends Widget_Base {
	...
	public function get_title() {
		return __( 'Latest Videos', 'vidiho-pro' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_title',
			[
				'label' => __( 'Latest Videos', 'vidiho-pro' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'raw'             => __( 'Displays a number of the latest (or random) videos, optionally from a specific category.', 'vidiho-pro' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'options' => vidiho_pro_get_terms( 'vidiho_pro_video_category' ),
			]
		);
		...
		$this->start_controls_section(
			'section_style',
			[
				'label' => __( 'Latest Videos Element Styles', 'vidiho-pro' ),
	...
	protected function render() {
		...
		echo do_shortcode( sprintf( '[latest-post-type post_type="%1$s" count="%2$s" columns="%3$s" random="%4$s" carousel="%5$s" taxonomy="%6$s" term_ids="%7$s"]',
			...
			$random,
			$carousel,
			'vidiho_pro_video_category',
			$term_id
		) );
		?>
		<script>
			jQuery(document).ready(function(){
				jQuery(document).trigger('elementor/render/latest_videos','#ci-lv-<?php echo esc_attr( $this->get_id() ); ?>');
			});
		</script>
		<?php
	}
