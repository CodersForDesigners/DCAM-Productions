...
if ( ! class_exists( 'Vidiho_Pro_Onboarding_Page' ) ) {

	/**
	 * Class used to generate the onboarding page.
	 */
	class Vidiho_Pro_Onboarding_Page {
		...
		public function page_setup() {
			...
			add_action( 'wp_ajax_vidiho_pro_dismiss_onboarding', array( $this, 'dismiss_onboarding' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_styles' ) );
			add_action( 'admin_menu', array( $this, 'register' ) );
			add_action( 'wp_ajax_install_vidiho_pro_plugin', array( $this, 'install_plugin' ) );
		}
		...
		public function onboarding_notice() {
			...
			?>
			<div class="notice notice-info is-dismissible vidiho-pro-onboarding-notice">
				<p>
					<?php
						/* translators: %1$s is the theme name. %2$s URL to onboarding page. */
						echo wp_kses( sprintf( __( 'Welcome to %1$s. Check out the <a href="%2$s">onboarding page</a> to get things started.', 'vidiho-pro' ),
							$this->theme_name,
			...
			wp_enqueue_script( 'vidiho-pro-onboarding-notice', get_template_directory_uri() . '/inc/onboarding/js/onboarding-notice.js', array(), $this->theme_version, true );

			$settings = array(
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'dismiss_nonce' => wp_create_nonce( 'vidiho-pro-dismiss-onboarding' ),
			);
			wp_localize_script( 'vidiho-pro-onboarding-notice', 'vidiho_pro_Onboarding', $settings );
		}
		...
		public function dismiss_onboarding() {
			check_ajax_referer( 'vidiho-pro-dismiss-onboarding', 'nonce' );
		...
		public function enqueue_admin_styles() {
			...
			wp_enqueue_style( 'vidiho-pro-onboarding', get_template_directory_uri() . '/inc/onboarding/css/onboarding-styles.css', array(), $this->theme_version );

			wp_enqueue_script( 'vidiho-pro-onboarding', get_template_directory_uri() . '/inc/onboarding/js/onboarding.js', array(
				'plugin-install',
				'updates',
			), $this->theme_version, true );

			wp_localize_script(
				'vidiho-pro-onboarding', 'vidiho_pro_onboarding', array(
					'onboarding_nonce'   => wp_create_nonce( 'onboarding_nonce' ),
					'ajaxurl'            => admin_url( 'admin-ajax.php' ),
					'template_directory' => get_template_directory_uri(),
					'activating_text'    => esc_html__( 'Activating', 'vidiho-pro' ),
					'activate_text'      => esc_html__( 'Activate', 'vidiho-pro' ),
					'installing_text'    => esc_html__( 'Installing...', 'vidiho-pro' ),
				)
			);
		}
		...
		public function render_page() {
			...
			if ( defined( 'VIDIHO_PRO_WHITELABEL' ) && VIDIHO_PRO_WHITELABEL ) {
				...
				$logo_url = ! empty( $this->data['logo_url'] ) ? $this->data['logo_url'] : 'https://www.cssigniter.com/themes/' . VIDIHO_PRO_NAME . '/';
			}
			?>
			<div class="wrap about-wrap vidiho-pro-onboarding-wrap full-width-layout">
				<h1><?php echo esc_html( $title ); ?></h1>

				<?php if ( ! empty( $this->data['description'] ) ) : ?>
					<p class="about-text"><?php echo wp_kses( $this->data['description'], vidiho_pro_get_allowed_tags( 'guide' ) ); ?></p>
				<?php endif; ?>
		...
		public function required_plugins() {
			?><h3><?php esc_html_e( 'The following plugins are required. Please install and activate them.', 'vidiho-pro' ); ?></h3><?php

			if ( ! current_user_can( 'install_plugins' ) ) :
				?><p><?php echo wp_kses( __( 'You do not have sufficient permissions to install plugins, please contact your administrator.', 'vidiho-pro' ), vidiho_pro_get_allowed_tags( 'guide' ) ); ?></p><?php
				return false;
			endif;
		...
		public function recommended_plugins() {
			?><h3><?php
				...
				echo esc_html( sprintf( __( 'The following plugins are not required but they will provide additional functionality to %s.', 'vidiho-pro' ), $this->theme_name ) );
			?></h3><?php
		...
		public function sample_content() {
			$check = $this->get_plugin_action( array( 'one-click-demo-import' => __( 'One Click Demo Import', 'vidiho-pro' ) ) );

			?><h3><?php esc_html_e( 'Import our sample content.', 'vidiho-pro' ); ?></h3><?php

			if ( in_array( $check['one-click-demo-import'], array( 'install-plugin', 'activate' ), true ) ) {
				?>
				<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-warning">
					<h4 class="box-title"><?php esc_html_e( 'Please note:', 'vidiho-pro' ); ?></h4>
					<p><?php esc_html_e( 'You need to install and activate One Click Demo Import before proceeding.', 'vidiho-pro' ); ?></p>
					<p><a class="button button-primary" href="<?php echo esc_url( get_admin_url( '', 'themes.php?page=' . $this->theme_slug . '-onboard' ) . '&tab=recommended_plugins' ); ?>"><?php esc_html_e( 'Click here to do it now.', 'vidiho-pro' ); ?></a></p>
				</div>
				<?php
			} else {
				?>
				<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-success">
					<h4 class="box-title"><?php esc_html_e( 'Good to go!', 'vidiho-pro' ); ?></h4>
					<p><?php esc_html_e( 'Now you can import the sample content and have your theme set up like the demo using the One Click Demo Import Plugin.', 'vidiho-pro' ); ?></p>
					<p><a class="button button-primary" href="<?php echo esc_url( get_admin_url( '', 'themes.php?page=pt-one-click-demo-import' ) ); ?>"><?php esc_html_e( 'Get Started', 'vidiho-pro' ); ?></a></p>
				</div>
				<?php
			}
		...
		public function support() {
			...
			?>
			<h3><?php esc_html_e( 'Here are a few useful links to get you started.', 'vidiho-pro' ); ?></h3>
			<div class="three-col">
				<?php
					...
					foreach ( $sections as $section_id => $section ) {
						?>
						<div class="col">
							<div class="vidiho-pro-onboarding-box vidiho-pro-support-tab-box-<?php echo esc_attr( $section_id ); ?>">
								<h4 class="box-title"><a href="<?php echo esc_url( $section['link_url'] ); ?>" target="_blank"><?php echo esc_html( $section['title'] ); ?></a></h4>
								<?php if ( ! empty( $section['description'] ) ) : ?>
									<p><?php echo wp_kses( $section['description'], vidiho_pro_get_allowed_tags( 'guide' ) ); ?></p>
								<?php endif; ?>
		...
		public function get_plugin_boxes( $plugins, $actions ) {
			foreach ( $actions as $slug => $action ) {
				if ( 'upload-plugin' === $action ) {
					...
					<div class="col">
						<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-warning">
							...
							<?php if ( ! empty( $plugins[ $slug ]['description'] ) ) : ?>
								<p class="box-description"><?php echo wp_kses( $plugins[ $slug ]['description'], vidiho_pro_get_allowed_tags( 'guide' ) ); ?></p>
							<?php endif; ?>
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<?php
									...
									echo esc_html( sprintf( __( 'The %s plugin was not found, click to install and activate.', 'vidiho-pro' ), $plugins[ $slug ]['title'] ) );
								?>
								</p>
								<p><a href="<?php echo esc_url( $link ); ?>" class="install-now button ajax-install-plugin" data-plugin-slug="<?php echo esc_attr( $slug ); ?>"><?php esc_html_e( 'Install &amp; activate ', 'vidiho-pro' ); ?></a></p>
							</div>	
						</div>
					</div>	
					<?php
					...
					<div class="col">
						<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-warning">
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<p><a data-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="install-now button"><?php esc_html_e( 'Install &amp; activate ', 'vidiho-pro' ); ?></a></p>
							</div>	
						</div>
					</div>	
					<?php
					...
					?>
					<div class="col">
						<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-info">
							<h4 class="box-title"><?php echo esc_html( $plugins[ $slug ]['title'] ); ?></h4>
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<?php
									/* translators: %s is the plugin name. */
									echo esc_html( sprintf( __( 'The %s plugin is installed but not active, click to activate.', 'vidiho-pro' ), $plugins[ $slug ]['title'] ) );
								?>
								</p>
								<p><a data-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="activate-now button button-primary"><?php esc_html_e( 'Activate ', 'vidiho-pro' ); ?></a></p>
							</div>	
						</div>
					</div>
					<?php
					...
					<div class="col">
						<div class="vidiho-pro-onboarding-box vidiho-pro-onboarding-box-success">
							...
							<?php
								/* translators: %s is the plugin name. */
								echo esc_html( sprintf( __( '%s is installed and activated.', 'vidiho-pro' ), $plugins[ $slug ]['title'] ) );
							?>
							</p>
						</div>
					</div>	
		...
		public function install_plugin() {
			if ( ! current_user_can( 'upload_plugins' ) ) {
				wp_die( __( 'You do not have sufficient permissions to install plugins on this site.', 'vidiho-pro' ) );
			}
			...
			$title    = sprintf( __( 'Installing Plugin from URL: %s', 'vidiho-pro' ), esc_html( $plugin_source_url ) );
		...
		public function default_data() {
			...
			return array(
				...
				// Required. The logo's link URL. Defaults to 'https://www.cssigniter.com/themes/' . VIDIHO_PRO_NAME . '/'
