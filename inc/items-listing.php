<?php
function vidiho_pro_post_type_listing_get_valid_columns_options( $post_type = false ) {
	$array = array(
		'min'   => 1,
		'max'   => 4,
		'range' => range( 1, 4 ),
	...
	return apply_filters( 'vidiho_pro_post_type_listing_valid_columns_options', $array, $post_type );
}

function vidiho_pro_post_type_listing_get_post_terms_classes( $post_id, $taxonomy = false ) {
	$terms_classes = array();
	if ( $taxonomy ) {
	...
	return apply_filters( 'vidiho_pro_post_type_listing_post_terms_classes', $terms_classes, $post_id, $taxonomy );
}
 ...
 * @return array
 */
function vidiho_pro_post_type_listing_get_template_data( $post_id, $post_type ) {

	$taxonomy = false;
	if ( function_exists( 'vidiho_pro_plugin_post_type_listing_taxonomy' ) ) {
		$taxonomy = vidiho_pro_plugin_post_type_listing_taxonomy( $post_type );
	}
	...
	$spacing = '';
	if ( apply_filters( 'vidiho_pro_post_type_listing_spacing_support', true, $post_type ) ) {
	...
	$data = apply_filters( 'vidiho_pro_post_type_listing_template_data', $data, $post_id, $post_type );
